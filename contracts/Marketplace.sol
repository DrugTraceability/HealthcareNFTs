...
contract Marketplace is ReentrancyGuard {
    ...
    enum NFTState {UnListed, Listed, Purchased, RedeemInitiated, EnRoute, Delivered, Disputed, Challenged}
    enum DisputeDecision {RejectReceiving, DamagedItem, NotDelivered}
    ...
    event Delisted(uint itemId, address indexed nft, uint tokenId, address indexed seller);
    ...
    event DisputeOpenned(address indexed Seller, address indexed nft, address indexed Buyer, uint tokenId, string  IPFShash);
    event DisputeSettlement(address indexed winner, address indexed nft, address indexed redeemer, uint tokenId, DisputeDecision _decision);
    ...
    event DisputeChallenged(address indexed Seller, address indexed nft, address indexed Buyer, uint tokenId, string  IPFShash);
    ...
    function makeItem(IERC721 _nft, uint _tokenId, uint _price) external payable nonReentrant { //IERC721 _nft takes the address of the nft and make it an nft instance, nonReentrant is from the imported reentrancyguard 
        uint securtiydeposit = _price;
        require(_price > 0, "Price must be greater than zero");
        require(msg.value >= securtiydeposit, "not enough Ether to cover the security deposit"); 
        // increment itemCount
        itemCount ++;
        // transfer nft and security deposit
        payable(address(this)).call{value: _price}; //The value of the NFT is transferred as a security deposit
    ...
    //Delist a product from the platform
    function delistItem(uint _itemId) external payable nonReentrant{
        Item storage item = items[_itemId];
        require(msg.sender == item.seller, "Only the seller of the item can delist it");
        require(!item.sold, "The item is already sold and cannot be delisted");
        item.nft.transferFrom(address(this), msg.sender, item.tokenId);
        item.NFTstate = NFTState.UnListed;
        payable(msg.sender).transfer(item.price); 
        //payable(msg.sender).call{value: item.price};
        emit Delisted(item.itemId, address(item.nft), item.tokenId, item.seller);
    }
    ...
    function purchaseItem(uint _itemId) external payable nonReentrant { //external to prevent accessing it from within the smart contract
        ...
        require(msg.value >= _totalPrice, "not enough ether to cover item price"); //_totalPrice is used in case there is a fee percentage
    ...
    function ClaimNFTValue(uint _itemCount) public{
        ...
        payable(msg.sender).transfer(2*item.price); //This includes the security deposit as well
    ...
    require(block.timestamp <= item.SellingTime + RedemptionPeriod, "Cannot redeem the NFT as the redemption period is over");
    require(msg.value >= item.price, "Not enough Ether to cover the security deposit");
    ...
    function OpenDispute(uint _itemCount, string memory _IPFShash) public {
        ...
        emit DisputeOpenned(item.seller, address(item.nft), item.nft.ownerOf(item.tokenId), item.tokenId, _IPFShash);
    ...
    function ChallengeDispute(uint _itemCount, string memory _IPFShash) public{
        ...
        emit DisputeChallenged(item.seller, address(item.nft), item.nft.ownerOf(item.tokenId), item.tokenId, _IPFShash);
    ...
    function DisputeFinalDecision(uint _itemCount, DisputeDecision _decision) public{
        Item storage item = items[_itemCount];
        address _winner;
        ...
        require(item.disputed  || item.NFTstate == NFTState.Challenged, "The item has not been disputed or challenged yet");

        
        if(_decision == DisputeDecision.RejectReceiving){ //This means the seller tried delivering but the buyer rejected
            _winner = item.seller;
            payable(_winner).transfer(2*item.price); //The seller gets twice the value of the item
            valueClaimed[item.itemId] = true; //This is to ensure that the seller does not reclaim the value of the NFT from the smart contract again
        ...
        if(_decision == DisputeDecision.DamagedItem){
            _winner = item.redeemer;
            payable(_winner).transfer(2*item.price); //The buyer gets twice the value of the item
            item.disputed = false;
            item.nft.transferFrom(address(this), address(0x000000000000000000000000000000000000dEaD) , item.tokenId); //The nft is burned because the item is damaged
        }

        if(_decision == DisputeDecision.NotDelivered){
            _winner = item.redeemer;
            payable(_winner).transfer(2*item.price); //The buyer gets twice the value of the item
